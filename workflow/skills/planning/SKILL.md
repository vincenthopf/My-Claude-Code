...
**IMPORTANT: Use pi dispatch for proposals, NOT Claude sub-agents.** Proposals are generated by Codex, not Claude. Read the dispatch patterns at `~/.claude/skills/workflow-research/codex-dispatch.md` for exact commands.

Dispatch two Codex High agents in parallel via pi using the Bash tool with `run_in_background: true`:
...
# Proposal B — same prompt, different output file
python3 ~/.claude/skills/pi-agent/scripts/pi-run.py \
  --prompt "[proposal prompt]" \
  --output .building/planning/proposal-b.md \
  --provider openai-codex --model gpt-5.4 \
  --thinking high \
  --tools read,grep,find,ls \
  --cwd "$(pwd)" \
...
Add `.building/design/design.md` and `.building/principles.md` to `--context` if they exist. Use `--tools read,grep,find,ls` — read-only, proposals don't modify the codebase. Set `timeout: 600000` on the Bash call for complex proposals.
