...
**Use pi-watch.sh to send prompts to Codex, NOT Claude sub-agents.** Proposals are generated by Codex.
...
Run the same prompt a second time telling Codex to write to `proposal-b.md` for an independent second proposal. Both run in background simultaneously.
