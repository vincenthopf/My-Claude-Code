...
**IMPORTANT: Use the workflow dispatch script, NOT Claude sub-agents.** Proposals are generated by Codex via pi.

Dispatch two proposals in parallel using the Bash tool with `run_in_background: true` and `timeout: 600000`:

```bash
# Proposal A
python3 ~/.claude/skills/workflow-research/dispatch.py proposal --cwd "$(pwd)" --id a

# Proposal B (run simultaneously as a second background task)
python3 ~/.claude/skills/workflow-research/dispatch.py proposal --cwd "$(pwd)" --id b
```

The dispatch script automatically:
- Uses Codex (openai-codex/gpt-5.4) with high thinking
- Passes the problem definition and principles as context
- Picks up design artifacts and project principles if they exist
- Gives read-only tool access (proposals don't modify the codebase)
- Writes to `.building/planning/proposal-a.md` and `.building/planning/proposal-b.md`

Each agent receives (via context files):
